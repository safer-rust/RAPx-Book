...
Owned heap analysis determines whether a Rust type owns heap-allocated memory. This information is essential for memory leak detection (Chapter 6.2) and other analyses that must distinguish stack-only types from those requiring deallocation. The module lives at `rapx/src/analysis/ownedheap_analysis/`.
...
Beyond the boolean heap-owner classification, the `Encoder` struct in `default.rs` produces an `OwnershipLayoutResult` for each field of a type. This is used by the memory leak detector (`rCanary`, Chapter 6.2) to determine, at runtime, which fields of a drop-in-progress struct need deallocation.
...
The test at `rapx/tests/ownedheap/heap_proxy/src/main.rs` defines five proxy structs:
...
- **Memory Leak Detection (Chapter 6.2)**: `rCanary` uses `OwnedHeapAnalyzer` to identify which types need deallocation tracking, and uses `Encoder::encode()` to determine field-level ownership layouts for runtime drop analysis.
- **SafeDrop (Chapter 6.1)**: Uses heap ownership information to reason about whether a pointer aliases memory subject to deallocation.
