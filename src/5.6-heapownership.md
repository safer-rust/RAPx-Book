...
Heap ownership analysis determines whether a Rust type owns heap-allocated memory. This information is essential for [memory leak detection](./6.2-memleak.md) and other analyses that must distinguish stack-only types from those requiring deallocation. The module lives at [`rapx/src/analysis/heap_ownership/`](https://github.com/safer-rust/RAPx/tree/main/rapx/src/analysis/heap_ownership/).
...
`HeapOwnershipAnalyzer::start()` in `default.rs` proceeds through four phases. First, it visits all MIR bodies reachable from the local crate, collects all ADT types, and records their `DefId`s. Then:
...
The test at [`rapx/tests/analyze/heapowner_proxy/src/main.rs`](https://github.com/safer-rust/RAPx/blob/main/rapx/tests/analyze/heapowner_proxy/src/main.rs) defines five proxy structs:
...
- **[Memory Leak Detection](./6.2-memleak.md)**: `rCanary` uses `HeapOwnershipAnalyzer` to identify which types need deallocation tracking, and uses `Encoder::encode()` to determine field-level ownership layouts for runtime drop analysis.
