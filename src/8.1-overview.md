...
The `--module <PATH>` flag restricts verification to functions whose def-path matches the given module (the crate prefix is matched flexibly, so `--module slice` matches `core::slice::*`).
