...
To approximate different access modes, we introduce two variants:
- an immutable view `Slice<T>` using `*const T`, and  
- a mutable view `SliceMut<T>` using `*mut T`.  

This distinction models raw pointer capabilities (read vs write), rather than Rust’s borrowing semantics. 
In particular, it does not capture aliasing or exclusivity guarantees enforced by `&[T]` and `&mut [T]`.
...
For each struct, we annotate its type invariants using a contract DSL, which can be used for verification.
