...
(1) **Audit unit extraction:** Segmenting Rust code into independent units to set analysis boundaries for verification and inference.
...
Since undefined behavior (UB) arises only from unsafe code, such code serves as a key indicator for soundness analysis.
Audit units are a foundational abstraction that define the boundaries of analysis for verification and inference.

For a given Rust crate, three typical scenarios arise when handling unsafe code:

* **Original unsafe code:** 
  (i) Unsafe code within a Rust crate with no additional internal unsafe code; 
  (ii) Unsafe APIs originating from external libraries. See [this document](https://safer-rust.github.io/rust-safety-standard/rust-safety-standard.html#23-design-choices) for more details.

* **Safety property delegation:** 
  An unsafe API that exposes the safety requirements of internal unsafe code.

* **Safety encapsulation:** 
  A safe API that fully enforces all necessary safety properties of internal unsafe code.

Each of these scenarios introduces natural boundaries where the propagation of unsafety can be either contained or observed. To effectively isolate, analyze, and verify unsafe code, we define audit units to enable focused and manageable safety audits.

## 2 Audit Unit Extraction

### Audit Unit Construction
