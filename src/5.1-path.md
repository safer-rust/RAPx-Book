...
Paths 1–4 all appear in the output; Path 5 never does. The divergence is driven by the `visited_sccs` cache (see [Section 5.1.2.3](#5.1.2.3-pruning-infeasible-paths)). Here is what happens during DFS enumeration:
...
Paths are stored in a `PathTree` (prefix-tree), keyed by `DefId`. See the [source](https://github.com/safer-rust/RAPx/blob/main/rapx/src/analysis/path/) for the full API — `PathGraph` for CFG+SCC+constraint checking, `PathEnumerator` for DFS enumeration with SCC caching.
