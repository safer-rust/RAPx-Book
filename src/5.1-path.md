...
Paths 1–4 all appear in the output. Path 5 (skip then body) is structurally valid but never generated: the `visited_sccs` cache (see [Section 5.1.2.3](#5.1.2.3-pruning-infeasible-paths)) blocks re-expansion of the inner SCC when the constraint hash is unchanged. The hash table below shows why — the first entry and re-entry via inner skip share the same fingerprint H₁, so the SCC is not re-expanded; re-entry via inner body carries extra compiler-injected constants from `bb6`–`bb8`, producing a different hash Hₑ that triggers a fresh expansion (yielding path 4, body then skip).
...
Paths are stored in a `PathTree` (prefix-tree), keyed by `DefId`. See the [source](https://github.com/safer-rust/RAPx/blob/main/rapx/src/analysis/path_analysis/) for the full API — `PathGraph` for CFG+SCC+constraint checking, `PathEnumerator` for DFS enumeration with SCC caching.
