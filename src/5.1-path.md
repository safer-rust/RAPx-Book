...
Why not 4 paths? Structurally the two `SwitchInt` terminators create 2 × 2 = 4 combinations, but `PathGraph::check_transition()` filters infeasible transitions incrementally during enumeration: before following a CFG edge into the next block, the constraint system checks whether the transition is consistent with accumulated state. 

The mechanism tracks discriminant constraints step by step along each path:

- `check_transition(cur, next, &mut constraints)` examines the edge `cur → next`. For `SwitchInt` terminators at `cur`, it records the discriminant's concrete value in a `constraints` map (e.g., taking `bb3` means `_1 = 1` for `Some`). The `disc_info.source_of` map links temporary locals like `_3` back to the source `_1`.
- When the same discriminant is tested again at `bb4`, `check_transition` checks the existing constraint: `bb3 → bb4 → bb5` is rejected because `_1 = 1` contradicts the `0` target at `bb5`; `bb2 → bb4 → bb6` is rejected because `_1 = 0` contradicts the `1` target at `bb6`. Only the two consistent combinations are inserted into the `PathTree`.
...
`check_transition` prunes all but two of these structural paths by tracking three constraints:

- **`x` is immutable**: once `_1 = 0` or `_1 = 1` is recorded at `bb2`, it persists. Any path that reverses the branch (C, B–A, D–A, D–C, etc.) is rejected.
- **`inner_retry`** is reset to `true` at `bb1` and toggled to `false` at `bb5`. Since `bb1` sets `_3 = const true`, the first `bb4` in any outer iteration sees `copy _3 = true`, so `bb4 → bb6` (B's exit) is rejected. This eliminates B as the first inner path in every outer iteration.
- **`outer_retry`** starts `true` (`bb0`) and is set `false` at `bb7`. The first `bb6` must take `otherwise → bb7`; any path trying `bb6 → bb8` on the first visit is rejected. This forces at least one outer retry and eliminates 1-iteration B and D paths.

Only two paths survive:

    Path [0, 1, 2, 3, 9]                              // x = true, A
    Path [0, 1, 2, 4, 5, 2, 4, 6, 7, 1, 2, 4, 5, 2, 4, 6, 8, 9]  // x = false, D–D

Example 1 prunes crossed enum branches in a DAG; Example 2 prunes loop paths by tracking immutable/constant assignments; Example 3's conditions produce fresh temporaries that are invalidated each iteration, so its constraint system provides no filtering.

### 5.1.3.3 Child SCC Context Caching

Reachability filtering runs incrementally during enumeration via `check_transition`: before each recursive step into a CFG successor, the constraint state is updated and the transition is validated, pruning infeasible branches early — at expansion time rather than only at terminal blocks. This eliminates the redundant prefix re-scanning that a post-enumeration checker would require.

The SCC tree introduces a second layer of pruning that operates alongside constraint filtering: the **`visited_sccs`** cache. When the outer SCC's DFS encounters a child SCC entry block, it asks: *has the accumulated constant state changed since the last time I entered this child?*

The mechanism is implemented in `dfs_scc_tree`. Before expanding a child SCC, it invokes `constraint_context(path)` to walk the entire path prefix and collect all `{local → constant_value}` pairs seen so far. These are hashed into a [`ConstraintHash`](https://github.com/safer-rust/RAPx/blob/main/rapx/src/analysis/path_analysis/graph.rs). The cache uses a [`SccKey`](https://github.com/safer-rust/RAPx/blob/main/rapx/src/analysis/path_analysis/graph.rs) keyed by `(entry, repeat, constraint)`. For `visited_sccs`, only `entry` and `constraint` are meaningful (the `repeat` field is unused in this context). If the same key has already been seen, the child SCC is **not** re-expanded — the DFS knows the child paths would be identical to those already enumerated.
...
Paths 1–4 all appear in the output. Path 5 is structurally valid — the outer SCC uses each distinct postfix once — but the DFS never generates it. The mechanism responsible is the `visited_sccs` cache described in Section 5.1.3.3. Before expanding `bb4`, `dfs_scc_tree` hashes the accumulated constant state along the path prefix. If the `SccKey { entry: bb4, constraint: hash }` has been cached before, the child SCC is not re-expanded.
