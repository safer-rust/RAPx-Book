...
entry   _1 = self (&[T]), _2 = index (I)
          ...
          init_parameters:
            ...
            path conditions: 0 <= len, len * size_of(T) <= isize::MAX
            _2: term = param_2 (fresh symbolic), no prov

          contract facts (requires + slice invariant):
            InBound(self, index)  →  _1.in_bounds
            NonNull(self)         →  _1.non_null            (already set)
            Init(self, T, len)    →  alloc(_1).initialized  (already set)
