...
**Built-in type invariants.** Every target is a method on the primitive `[T]` (no `#[rapx::invariant]` struct), so RAPx relies on its built-in handling for the slice receiver. This runs in two independent places:

1. **Entry (`init_parameters`)** — for a `&[T]` / `&mut [T]` parameter `self`, the VM establishes the slice's full validity as symbolic state without pre-deciding what will need re-proving:
   - the reference value gets the flags `non_null`, `aligned`, `init`;
   - a data allocation is created with `align = align_of(T)`, `size = len * size_of(T)`, `initialized = true`, `element_ty = T`;
   - the length bounds are asserted as path conditions: `0 <= len` and `len * size_of(T) <= isize::MAX`.

2. **Return (`slice_invariant_properties`)** — the properties re-proved at every path endpoint are exactly:
...
These three are re-proved because they are what unsafe code in the body can still break: a raw write or `drop_in_place` can de-initialize the elements (`Init`) or free the memory (`Alive`). `Align` and the length bounds are not in the re-proof set — they are fixed at entry (the reference's address and length are immutable) and are instead enforced where a new pointer or length is introduced: `&*ptr` (`Ptr2Ref = Init + Align + Alias`) and `from_raw_parts` (`Align(data, T)` + `ValidNum(size_of(T) * len <= isize::MAX)`).
...
```text
entry   _1 = self (&[T]), _2 = index (I)   — bound by init_parameters
          _1: term/prov → data alloc, flags { non_null, aligned, init }
