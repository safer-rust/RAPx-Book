...
`Align(self, T)` is deliberately omitted: the data pointer's address is immutable (it never changes across the call) and alignment is enforced at the deref / `from_raw_parts` sites instead, since `check_align` cannot discharge it for a generic element type. Like a user `#[rapx::invariant]` struct, the three invariants are assumed at entry and re-proved at return on every path endpoint, so a mutation that breaks the slice (e.g. an out-of-bounds raw write) is caught even without an explicit invariant.
