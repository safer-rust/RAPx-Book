...
On top of these, the `&[T]` receiver `self` carries a built-in slice invariant (`NonNull` + `Init` + `Alive`) that is assumed at entry and re-proved at every path endpoint.
...
**Entry assumptions.** RAPx synthesizes the following caller contracts at function entry:

| Contract | Meaning |
|----------|---------|
| `InBound(self, index)` | `0 <= index < self.len()` — from the `#[rapx::requires]` annotation |
| `NonNull(self)` | `self as usize != 0` |
| `Init(self, T, len(self))` | all `len(self)` elements are initialized |
| `Alive(self)` | `*self` outlives the return |

**Verification.** RAPx reports four checkpoints on the two whole-CFG paths — the normal path `bb0 → bb1` and the unwind path `bb0 → bb2` — and proves all of them (`result: SOUND`):

| CP | Block | MIR statement | Contract to prove | Discharged by |
|----|-------|---------------|-------------------|---------------|
| (1) | bb0 | `_4 = get_unchecked(move _5, move _6)` | `InBound(_6, _5)` = `0 <= index < len` | entry `InBound(self, index)`; `_6` ← `&raw const (*_1)` (slice), `_5` ← `move _2` (index) |
| (2) | bb1 | `_3 = &(*_4)` | `Init(_4, T, 1)` + `Align(_4, T)` + `Alias(_4)` | `Init`/`Align` inherited from `Init(self, T, len(self))`; `Alias` from the shared `&self` receiver |
| (3) | bb1 | `return` (normal endpoint) | `NonNull(_1)` + `Init(_1, T, len)` + `Alive(_1)` | `self` is only read, never mutated, so the slice invariant is preserved |
| (4) | bb2 | `resume` (unwind endpoint) | `NonNull(_1)` + `Init(_1, T, len)` + `Alive(_1)` | same — the unwind path also leaves `self` untouched |

The raw output, verbatim:

```text
[rapx::verify] function: slice::<impl [T]>::get_unchecked
  --- unsafe checkpoints ---
      unsafe checkpoint: bb1 -> raw-ptr-deref
        path [0, 1]:
          ...
          ├── Align | Proved
          └── [hazard] Alias | Proved
      unsafe checkpoint: bb0 -> core::slice::index::SliceIndex::get_unchecked
        path [0]:
          InBound | Proved
      unsafe checkpoint: bb2 -> type-invariant(bb2)
        path 0, 2:
          ├── NonNull | Proved
          ├── Init | Proved
          └── Alive | Proved
      unsafe checkpoint: bb1 -> type-invariant(bb1)
        path 0, 1:
          ├── NonNull | Proved
          ├── Init | Proved
          └── Alive | Proved
...
|----|--------------|-------------------|-----------|------------|
| entry | — | — | — | assume: `InBound(index_access(_1, _2))`, `_1` slice invariant |
| — | `_8 = &raw mut (*_1)` | — | alias: `_1`, `_8` | — |
| (1) | `_6 = get_unchecked_mut(move _7, move _8)` | `InBound(_8, _7)` | provenance: `_6 → _8 → _1` | ← `InBound(index_access(_1, _2))` |
...
| ret | `return` | type-invariant: `NonNull` + `Init` + `Alive` | — | ← `_1` invariant |
