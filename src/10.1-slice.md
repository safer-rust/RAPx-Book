...
**Entry.** The entry contract `InBound(index_access(self, index))` and the `&[T]` slice invariant.

**VM execution.** Each statement updates the symbolic state; a checkpoint (CP) asserts a Contract, discharged by the Constraint:

| CP | MIR statement | VM effect | Contract | Constraint |
|----|--------------|-----------|----------|------------|
| — | `_6 = &raw const (*_1)` | alias: `_1`, `_6` | — | — |
| (1) | `_4 = get_unchecked(move _5, move _6)` | provenance: `_4 → _6 → _1` | `InBound(slice, self)` | ← entry `InBound(index_access(self, index))` |
| (2) | `_3 = &(*_4)` | reference from raw `_4` | `Ptr2Ref` | `Init`/`Align` ← invariant; `Alias` ← shared |
...
**Entry.** The entry contract `InBound(index_access(self, index))` and the `&mut [T]` slice invariant (same as `&[T]`, plus exclusive access).
...
| (1) | `_6 = get_unchecked_mut(move _7, move _8)` | provenance: `_6 → _8 → _1` | `InBound(slice, self)` | ← entry `InBound(index_access(self, index))` |
| (2) | `_5 = &mut (*_6)` | reference from raw `_6` | `Ptr2Ref` | `Init`/`Align` ← invariant; `Alias` ← exclusive |
