...
**Verification.** The path, top to bottom:

| CP | MIR statement | Contract to prove | VM effect | Constraint |
|----|--------------|-------------------|-----------|------------|
| entry | — | — | — | assume: `InBound(index_access(_1, _2))`, `_1` slice invariant (`NonNull` + `Init` + `Alive`) |
...
| (2) | `_3 = &(*_4)` | `Ptr2Ref`: `Init` + `Align` + `Alias` | reference from raw `_4` | `Init`/`Align` ← `_1` invariant; `Alias` ← shared `&self` |
| — | `_0 = &(*_3)` | — | reborrow → return slot | — |
| ret | `return` | type-invariant: `NonNull` + `Init` + `Alive` | — | ← `_1` invariant (`self` only read) |
| unwind | `resume` | type-invariant: `NonNull` + `Init` + `Alive` | — | ← `_1` invariant (`self` untouched) |
