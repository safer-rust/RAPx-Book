...
_4 = get_unchecked(move _5, move _6)                    [checkpoint (1)]
          check  InBound(_6, _5)   ←  _5.term < len              Proved
                              (SMT: _5.term = _2.term (move _2); _6 aliases _1, so len(_6) = len(_1)
                               = data_alloc.slice_len = len;
                               thus _5.term < len ⟺ _2.term < len — the `InBound(_1, _2)` entry fact)
          VM  _4.term = _6.term + _5.term*sizeof_T, _4.prov = (_6.alloc_id, _6.offset + _5.term*sizeof_T)

_3 = &(*_4)                                             [checkpoint (2)]
          check  Init(_4, T, 1)      ←  data_alloc.initialized = true                                     Proved
          check  NonNull(_4)         ←  flag: non_null (inherited from _1)                                Proved
          check  Allocated(_4, T, 1) ←  SMT: (_5.term+1)*sizeof_T ≤ len*sizeof_T                               Proved
          check  InBound(_4, T, 1)   ←  SMT: same bound, _5.term ≥ 0 — both by `_5.term < len` and sizeof_T ≥ 1     Proved
...
| ret | `return` | type-invariant: `NonNull` + `Align` + `ValidNum(size_of(T)*len <= isize::MAX)` + `Allocated` + `Init` | — | ← `_1` invariant |
