...
**Verification Targets.** Same as `get_unchecked`; the only difference is that the `&mut *` deref carries one extra obligation:

```rust
#[rapx::requires(Alias(ptr))]  // exclusive access required by `&mut`
```

**Soundness.** Discharged exactly as in `get_unchecked`; the extra `Alias` obligation is supplied by the exclusive `&mut [T]` receiver.
