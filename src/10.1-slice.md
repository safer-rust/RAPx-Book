...
(2) the raw-pointer dereference `&*`, which creates a *shared* reference from a raw pointer (`Ptr2Ref`):

```rust
// implicit contract of the reference creation `&*ptr`
#[rapx::requires(Init(ptr, T, 1))]
#[rapx::requires(Align(ptr, T))]
#[rapx::requires(Alias(ptr, ret))]  // the new shared reference must not conflict with a mutable alias
```

**Soundness.** Target (1): `SliceIndex::get_unchecked` requires `InBound(slice, self)` — exactly the entry precondition `InBound(index_access(self, index))`, so it is discharged directly. Target (2): `get_unchecked` returns the slice's data pointer advanced by the (in-bounds) index; the in-bounds index keeps the returned pointer inside the allocation, and `Init`/`Align` are inherited from the slice type invariant. The `Alias` hazard is auto-discharged: the source `&[T]` is itself a shared reference, so no mutable alias can coexist with it — shared-from-shared is always sound (even for a non-`Copy` `T` such as `T: Drop`).
...
**Verification Targets.** Same as `get_unchecked`; the only difference is that `&mut *` creates a *mutable* reference, whose `Alias` obligation is now exclusive (no other alias at all):

```rust
#[rapx::requires(Alias(ptr, ret))]  // exclusive access required by `&mut`
```
