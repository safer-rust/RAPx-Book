...
**Symbolic resolution.** `$self` instantiates to the slice reference and `$elem` to the element type `T`. For a generic `T`, its size and alignment are unknown, so `size_of($elem)` is the symbolic `sizeof_T` and `align_of($elem)` the symbolic `align_T` (both ≥ 1, linked by the layout constraint `sizeof_T % align_T == 0`); `len($self)` is the symbolic length `len`. Substituting, the five atoms become the facts the traces cite directly: `NonNull(self)`, `Align(self, T)` (i.e. `self % align_T == 0`), `len * sizeof_T ≤ isize::MAX`, `Allocated(self, T, len)`, and `Init(self, T, len)`.

`NonNull` and `Align` hold for every slice — even an empty one, whose dangling data pointer (`NonNull::dangling`) is still non-null and properly aligned (Rust requires alignment for size-0 access). `Allocated` and `Init` also hold unconditionally: an empty slice (`len == 0`) has zero elements, so both are vacuous.
