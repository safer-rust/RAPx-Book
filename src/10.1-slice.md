...
entry   _1 = self (&[T]), _2 = index (I)
          ...
          init_parameters:
            data_alloc: align = align_of(T), size = len * sizeof_T, element_ty = T, initialized = true
            _1: term = data_alloc.base, prov = (data_alloc, 0), flags { non_null, aligned, init }
               slice type invariant (JSON `[T]` entry, re-proved at return):
                 NonNull(_1)
                 Align(_1, T)
...
_3 = &(*_4)                                             [checkpoint (2)]
          ...
          check  Align(_4, T)        ←  flag: align_n (inherited via add; sizeof_T is a multiple of align_of(T))   Proved
