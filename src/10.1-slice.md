...
**Built-in type invariants.** Every target is a method on the primitive `[T]` (no `#[rapx::invariant]` struct), so RAPx relies on its built-in invariants for the slice receiver. For a `&[T]` / `&mut [T]` parameter `self`, `slice_invariant_properties` synthesizes exactly three invariants:

```rust
// built-in slice invariant, synthesized for a `&[T]` / `&mut [T]` receiver
#[rapx::invariant(NonNull(self))]
#[rapx::invariant(Init(self, T, len(self)))]
#[rapx::invariant(Alive(self))]
```

`Align(self, T)` and the `ValidNum` length bounds are deliberately omitted — they are immutable entry facts the body cannot break, not re-provable invariants:

- `Align` — the data pointer's address is fixed, so alignment cannot change across the call. Where it matters it is enforced at the use site instead: `&*ptr` (`Ptr2Ref = Init + Align + Alias`) and `from_raw_parts` (`Align(data, T)`). (`check_align` cannot discharge it for a generic element type either.)
- `ValidNum` (`0 <= len` and `size_of(T) * len <= isize::MAX`) — `len` is non-negative by type and immutable, and the bounds are asserted once at entry as path conditions (with the data allocation sized `len * size_of(T)`). A fresh length can only be introduced by `from_raw_parts(ptr, len)`, which explicitly requires `ValidNum(size_of(T) * len <= isize::MAX)`.

Like a user `#[rapx::invariant]` struct, the three remaining invariants (`NonNull`, `Init`, `Alive`) are assumed at entry and re-proved at return on every path endpoint, so a mutation that breaks the slice (e.g. an out-of-bounds raw write or `drop_in_place`) is caught even without an explicit invariant.
...
The `#[rapx::requires]` and slice-invariant facts are lowered at entry into flags on `_1` and marks on its allocation. Each checkpoint is discharged by reading those flags, or by SMT over the allocation's `base`/`size`/`align`. The path, step by step:

```text
entry   assume (facts → flags)
          InBound(self, index)  →  _1.in_bounds
          NonNull(self)         →  _1.non_null
          Init(self, T, len)    →  alloc(_1).initialized
          Alive(self)           →  alloc(_1).alive_assumed
...
_4 = get_unchecked(move _5, move _6)                    [checkpoint (1)]
          ...
          check  InBound(_6, _5)  ←  _6.in_bounds flag             Proved
...
return                                                   [type-invariant]
          check  NonNull(_1), Init(_1, T, len), Alive(_1)  ←  _1 unchanged   Proved
