...
Like a user `#[rapx::invariant]` struct, these are assumed at entry and re-proved at return on every path endpoint. The return check re-proves `NonNull(ptr)`, `Init(ptr, T, len)`, and `Alive(ptr)` — the properties unsafe code can still break through a raw-pointer write or `drop_in_place`. `Align` and the `ValidNum` length bounds are skipped: the reference's address and length are fixed.
