...
**Symbolic resolution.** `$self` instantiates to the slice reference and `$elem` to the element type `T`. For a generic `T`, the element size and alignment are *shared* symbolic constants `sizeof_T` and `align_T` (both ≥ 1, linked by the layout constraint `sizeof_T % align_T == 0`). The slice length `len` is *not* shared: it is a fresh symbolic value per slice, materialized on the data allocation. The atoms therefore resolve to:
...
_3 = &(*_4)                                             [checkpoint (2)]
          check  Init(_4, T, 1)      ←  flag: alloc(_4).initialized                                     Proved
          ...
          check  Align(_4, T)        ←  align_of(T) = 1 (trait-bound minimum for the unconstrained T)   Proved
