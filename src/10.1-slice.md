...
As with a user `#[rapx::invariant]` struct, these are **assumed at entry and re-proved at return**: a `&mut [T]` method that mutates the slice must leave the receiver still non-null, live, and fully initialized. The return check re-proves the *breakable* subset — `NonNull(ptr)`, `Init(ptr, T, len)`, and `Alive(ptr)` — as a `type-invariant(bbN)` checkpoint on every path endpoint. `Align` and the two `ValidNum` length bounds are omitted from the return check because they are immutable (the data pointer's address and the slice length never change) and are already enforced at deref / `from_raw_parts` sites.
...
- (2) `from_raw_parts`'s `ValidPtr`/`InBound`/`ValidNum`: `len % N == 0` gives `len = new_len * N`, so the byte size `size_of([T; N]) * new_len` equals the slice's own size. `NonNull`/`Align`/`Init`/`Alive`/`Alias` are inherited from the slice invariant. The `cast` re-interprets `T` as `[T; N]`, layout-identical by construction.
...
(1) `indices.get_unchecked(i)` — the slice `get_unchecked` on `[I; N]`, requiring `InBound(index_access(indices, i))`;
...
**Entry.** The preconditions `InBound(index_access(self, indices))` and `NonOverlap(indices)`, plus the `&mut [T]` slice invariant.

**Property check.**

- (1) `indices.get_unchecked(i)`: the loop index `i` is `< N` by construction → `InBound` discharged.
- (2) `slice.get_unchecked_mut(idx)`: discharged by `InBound(index_access(self, indices))`.
