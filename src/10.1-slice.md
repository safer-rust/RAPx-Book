          ...
          init_parameters:
            ...
            _1.term = data_alloc.base, _1.prov = (data_alloc, 0), _1.flags = { non_null, aligned, init, in_bounds }
               slice type invariant (JSON `[T]` entry):
                 _1.term ≠ 0                    (NonNull)
                 _1.term % align_T == 0         (Align)
                 len * sizeof_T ≤ isize::MAX    (ValidNum)
                 Allocated(_1, T, len)
...
_3 = &(*_4)                                             [checkpoint (2)]
          ...
          check  InBound(_4, T, 1)   ←  SMT: assert `(_5.term+1)*sizeof_T > len*sizeof_T` ∨ `_5.term*sizeof_T < 0`
                                            — first unsat by `_5.term < len`, second unsat by `_5.term ≥ 0` (usize) ⇒ UNSAT    Proved
          check  Align(_4, T)        ←  align = align_T (symbolic); _4.align_n = align_T (inherited via _6 from data_alloc.align), so align_n ≥ align holds trivially    Proved
          check  Alias(_4)           ←  (hazard) _4 traces to &self (shared borrow) → aliasing safe       Proved
...
return                                                   [type-invariant]
          check  _1.term ≠ 0, _1.term % align_T == 0, len * sizeof_T ≤ isize::MAX,
                 Allocated(_1, T, len), Init(_1, T, len)  ←  _1 unchanged; Align re-proved against the symbolic align_T   Proved
...
**Verification.** Identical to `get_unchecked`, on the mutable receiver (`&mut [T]`, `&mut *`):

```text
entry   _1 = self (&mut [T]), _2 = index (I)

          init_parameters:
            data_alloc.align = align_T, data_alloc.slice_len = len, data_alloc.size = len * sizeof_T, data_alloc.element_ty = T, data_alloc.initialized = true
            _1.term = data_alloc.base, _1.prov = (data_alloc, 0), _1.flags = { non_null, aligned, init, in_bounds }
            _2.term = param_2 (fresh symbolic)
...
_8 = &raw mut (*_1)
          VM  _8.term = _1.term, _8.prov = _1.prov, _8.flags = { non_null, in_bounds }, _8.align_n = data_alloc.align = align_T

_6 = get_unchecked_mut(move _7, move _8)                  [checkpoint (1)]
          check  InBound(_8, _7)   ←  SMT: assert entry `_2.term < len`; assert ¬(`_7.term < len`); `_7.term` ≡ `_2.term` ⇒ UNSAT    Proved
          VM  _6.term = _8.term + _7.term*sizeof_T, _6.prov = (data_alloc, _8.offset + _7.term*sizeof_T), _6.flags = { non_null, in_bounds }

_5 = &mut (*_6)                                           [checkpoint (2)]
          check  Init(_6, T, 1)      ←  data_alloc.initialized = true                                     Proved
          check  NonNull(_6)         ←  non_null ∈ _6.flags                                             Proved
          check  Allocated(_6, T, 1) ←  SMT: assert ¬(`(_7.term+1)*sizeof_T ≤ len*sizeof_T`)
                                            — unsat by `_7.term < len`, `sizeof_T ≥ 1` ⇒ UNSAT    Proved
          check  InBound(_6, T, 1)   ←  SMT: assert `(_7.term+1)*sizeof_T > len*sizeof_T` ∨ `_7.term*sizeof_T < 0`
                                            — first unsat by `_7.term < len`, second unsat by `_7.term ≥ 0` (usize) ⇒ UNSAT    Proved
          check  Align(_6, T)        ←  align = align_T (symbolic); _6.align_n = align_T (inherited via _8 from data_alloc.align), so align_n ≥ align holds trivially    Proved
          check  Alias(_6)           ←  (hazard) _6 traces to &mut self (exclusive) → aliasing safe       Proved
          VM  _5.term = _6.term, _5.prov = _6.prov, _5.flags = { non_null, aligned, init, in_bounds }

_4 = &mut (*_5), _3 = &mut (*_4), _0 = &mut (*_3)
          VM  reborrows → return slot

return                                                    [type-invariant]
          check  NonNull + Align + ValidNum(len*sizeof_T ≤ isize::MAX) + Allocated(_1, T, len) + Init(_1, T, len)  ←  _1 unchanged    Proved
...
_17 = from_raw_parts_mut(ptr.add(as_mut_ptr(_1), _2), _21) [checkpoint (2), right [mid, len)]
          check  NonNull + ValidPtr + Init + Alive + Alias + Align + ValidNum  ←  mid ≤ len    Proved
...
_51 = ptr.add(rest.as_ptr(), len − ts_len)
_50 = raw::from_raw_parts(_51, _39)                              [checkpoint (2), tail [T]]
...
_53 = from_raw_parts_mut(rest.as_mut_ptr().add(rest_len − ts_len), ts_len)   [checkpoint (2), tail [T]]
          check  NonNull + ValidPtr + Init + Alive + Alias + Align + ValidNum  ←  range analysis      Proved
...
_17 = ptr.sub(end, _2)
_16 = from_raw_parts_mut(_17, _2)                               [checkpoint (1), back_half]
...
# loop iteration (unrolled):
_23 = ptr.add(as_mut_ptr(_1), next_read)                       // ptr_read
_26 = ptr.add(as_mut_ptr(_1), next_write − 1)                  // prev_ptr_write
...
_31 = ptr.add(_29, src_start)                                  // src_ptr
_34 = ptr.add(_29, dest)                                       // dest_ptr

_37 = ptr::copy(_31, _34, count)                               [checkpoint]
