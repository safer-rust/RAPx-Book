          ...
          init_parameters:
            ...
            _1: term = data_alloc.base, prov = (data_alloc, 0), flags { non_null, aligned, init }
               slice type invariant (JSON `[T]` entry):
                 NonNull(_1)
                 Align(_1, T)                (align_of(T) = align_T, the shared symbolic element alignment)
                 len(_1)*size_of(T) ≤ isize::MAX   (len(_1) = len, the materialized slice length; size_of(T) = sizeof_T, the shared symbolic element size)
                 Allocated(_1, T, len(_1))         (vacuous for len == 0)
                 Init(_1, T, len(_1))              (vacuous for len == 0)
