...
**Verification.** Locals: `_1` = `self` (`&[T]`), `_2` = `index`. The path, top to bottom:

| CP | MIR statement | VM effect | Contract to prove | Constraint |
|----|--------------|-----------|----------|------------|
| entry | — | — | — | assume: `InBound(index_access(_1, _2))`, `_1` slice invariant |
| — | `_6 = &raw const (*_1)` | alias: `_1`, `_6` | — | — |
| (1) | `_4 = get_unchecked(move _5, move _6)` | provenance: `_4 → _6 → _1` | `InBound(_6, _5)` | ← `InBound(index_access(_1, _2))` |
| (2) | `_3 = &(*_4)` | reference from raw `_4` | `Ptr2Ref` | `Init`/`Align` ← `_1` invariant; `Alias` ← shared |
...
| (1) | `_6 = get_unchecked_mut(move _7, move _8)` | provenance: `_6 → _8 → _1` | `InBound(_8, _7)` | ← `InBound(index_access(_1, _2))` |
| (2) | `_5 = &mut (*_6)` | reference from raw `_6` | `Ptr2Ref` | `Init`/`Align` ← `_1` invariant; `Alias` ← exclusive |
