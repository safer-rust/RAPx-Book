...
**Symbolic resolution.** `$self` instantiates to the slice reference and `$elem` to the element type `T`. For a generic `T`, the element size and alignment are *shared per-type symbolic constants* `sizeof_T` and `align_T` (both ≥ 1, linked by the layout constraint `sizeof_T % align_T == 0`); the slice length is the *materialized* `len` (`slice_len`, one fresh constant per slice). The atoms therefore resolve to:
