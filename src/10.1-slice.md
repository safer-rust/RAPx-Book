...
**VM execution.** Each statement updates the symbolic state; at a checkpoint (CP) the verifier additionally discharges the property in the last column:

| CP | MIR statement | VM effect | Property check |
|----|--------------|-----------|----------------|
| — | `_6 = &raw const (*_1)` | `_6` aliases `_1` (same allocation) | — |
| (1) | `_4 = get_unchecked(move _5, move _6)` | provenance `_4 → _6 → _1` | `InBound(slice, self)` — by the entry `InBound(index_access(self, index))` |
| (2) | `_3 = &(*_4)` | reference from raw `_4` | `Ptr2Ref` = `Init` + `Align` + `Alias` — `Init`/`Align` from the slice invariant, `Alias` from the shared source |
| — | `_0 = &(*_3)` | reborrow → return slot | — |
...
| CP | MIR statement | VM effect | Property check |
|----|--------------|-----------|----------------|
| — | `_8 = &raw mut (*_1)` | `_8` aliases `_1` (same allocation) | — |
| (1) | `_6 = get_unchecked_mut(move _7, move _8)` | provenance `_6 → _8 → _1` | `InBound(slice, self)` — by the entry `InBound(index_access(self, index))` |
| (2) | `_5 = &mut (*_6)` | reference from raw `_6` | `Ptr2Ref` = `Init` + `Align` + `Alias` — `Init`/`Align` from the slice invariant, `Alias` from the exclusive receiver |
