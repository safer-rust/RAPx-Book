...
1. **Entry (`init_parameters`)** — for a `&[T]` / `&mut [T]` parameter `self`, the VM establishes the slice's full validity as symbolic state without pre-deciding what will need re-proving:
   - the slice-invariant facts (`NonNull`/`Init`/`Alive`): flags `non_null`, `init`, and the allocation marks `initialized = true`, `alive_assumed = true`;
   - the entry-fixed facts (not re-proved): the flag `aligned` (`Align`) and the path conditions `0 <= len` and `len * size_of(T) <= isize::MAX` (length bounds);
...
On top of these, the `&[T]` receiver `self` carries a built-in slice invariant (`NonNull` + `Init` + `Alive`) that `init_parameters` establishes at entry and that is re-proved at every path endpoint.
...
The `#[rapx::requires]` fact (`InBound`) is lowered at entry into the `in_bounds` flag on `_1`, while the built-in slice invariant (`NonNull`/`Init`/`Alive`) is established by `init_parameters` itself and re-proved at return. Each checkpoint is discharged by reading those flags, or by SMT over the allocation's `base`/`size`/`align`. The path, step by step:
...
entry   _1 = self (&[T]), _2 = index (I)
          ...
          init_parameters:
            ...
            data alloc: align = align_of(T), size = len * size_of(T), element_ty = T, initialized = true, alive_assumed = true
            _2: term = param_2 (fresh symbolic), no prov

          (re-proved at return: non_null → NonNull, init + initialized → Init, alive_assumed → Alive)
          (entry-fixed, not re-proved: aligned → Align, 0 <= len and len*size_of(T) <= isize::MAX → length bounds)
