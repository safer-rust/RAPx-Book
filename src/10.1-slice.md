...
|----|--------------|-------------------|-------------------------------------|------------|
| entry | — | — | — | `InBound(self, index)` → `_1.in_bounds`; `NonNull(self)` → `_1.non_null`; `Init(self, T, len)` → alloc `.initialized`; `Alive(self)` → alloc `.alive_assumed` |
| — | `_6 = &raw const (*_1)` | — | `_6.term = _1.term`; `_6.prov = _1.prov`; `non_null`, `in_bounds` | — |
| (1) | `_4 = get_unchecked(move _5, move _6)` | `InBound(_6, _5)` | `_4.term = _6.term`; `_4.prov = _6.prov` (`ReturnAliasArg`) | `_6.in_bounds` flag (set on `_1` at entry, carried by `&raw const`) |
| (2) | `_3 = &(*_4)` | `Ptr2Ref`: `Init` + `Align` + `Alias` | `_3.term = _4.term`; `_3.prov = _4.prov`; `non_null`, `aligned`, `init` | `Init` ← alloc `.initialized`; `Align` ← alloc `.align` + `offset % align == 0`; `Alias` ← shared view of `&self` |
| — | `_0 = &(*_3)` | — | `_0.term = _3.term`; `_0.prov = _3.prov`; `non_null`, `aligned`, `init` | — |
| ret | `return` | type-invariant: `NonNull` + `Init` + `Alive` | — | `_1` flags / allocation unchanged (`self` only read) |
| unwind | `resume` | type-invariant: `NonNull` + `Init` + `Alive` | — | `_1` flags / allocation unchanged (`self` untouched) |
