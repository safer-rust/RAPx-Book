...
entry   _1 = self (&[T]), _2 = index (I)
          ...
          init_parameters:
            data_alloc: align = align_T, slice_len = len, size = len * sizeof_T, element_ty = T, initialized = true
            _1: term = data_alloc.base, prov = (data_alloc, 0), flags { non_null, aligned, init }
               slice type invariant (JSON `[T]` entry):
                 NonNull(_1)
                 Align(_1, T)                (_1 % align_T == 0)
                 len * sizeof_T ≤ isize::MAX
                 Allocated(_1, T, len)       (vacuous for len == 0)
                 Init(_1, T, len)            (vacuous for len == 0)
...
_4 = get_unchecked(move _5, move _6)                    [checkpoint (1)]
          ...
          VM  _4: term = _6.term + _5*sizeof_T, prov = (_6.alloc_id, _6.offset + _5*sizeof_T)
