...
The `#[rapx::requires]` and slice-invariant facts are not kept as logical axioms — they are lowered at entry into flags on `_1` and marks on its allocation; each checkpoint is then discharged by reading those flags, or by SMT over the allocation's `base`/`size`/`align`:

| CP | MIR statement | Contract to prove | VM record (`term` / `prov` / flags) | Constraint |
|----|--------------|-------------------|-------------------------------------|------------|
| entry | — | — | — | `InBound(self, index)` → `_1.in_bounds`<br>`NonNull(self)` → `_1.non_null`<br>`Init(self, T, len)` → alloc `.initialized`<br>`Alive(self)` → alloc `.alive_assumed` |
| — | `_6 = &raw const (*_1)` | — | `_6.term = _1.term`<br>`_6.prov = _1.prov`<br>flags: `non_null`, `in_bounds` | — |
| (1) | `_4 = get_unchecked(move _5, move _6)` | `InBound(_6, _5)` | `_4.term = _6.term`<br>`_4.prov = _6.prov` (`ReturnAliasArg`) | `_6.in_bounds` flag (set on `_1` at entry, carried by `&raw const`) |
| (2) | `_3 = &(*_4)` | `Ptr2Ref`:<br>`Init`<br>`Align`<br>`Alias` | `_3.term = _4.term`<br>`_3.prov = _4.prov`<br>flags: `non_null`, `aligned`, `init` | `Init` ← alloc `.initialized`<br>`Align` ← alloc `.align` + `offset % align == 0`<br>`Alias` ← shared view of `&self` |
| — | `_0 = &(*_3)` | — | `_0.term = _3.term`<br>`_0.prov = _3.prov`<br>flags: `non_null`, `aligned`, `init` | — |
| ret | `return` | type-invariant:<br>`NonNull`<br>`Init`<br>`Alive` | — | `_1` flags / allocation unchanged (`self` only read) |
| unwind | `resume` | type-invariant:<br>`NonNull`<br>`Init`<br>`Alive` | — | `_1` flags / allocation unchanged (`self` untouched) |
