...
_4 = get_unchecked(move _5, move _6)                    [checkpoint (1)]
          ...
          VM  _4.term = _6.term + _5.term*sizeof_T, _4.prov = (data_alloc, _6.offset + _5.term*sizeof_T)

_3 = &(*_4)                                             [checkpoint (2)]
          check  Init(_4, T, 1)      ←  data_alloc.initialized = true — whole slice data is initialized at entry, so the element at offset _5.term*sizeof_T is covered    Proved
          check  NonNull(_4)         ←  flag: non_null (inherited from _6)                                Proved
          check  Allocated(_4, T, 1) ←  SMT: (_5.term+1)*sizeof_T ≤ len*sizeof_T                               Proved
          check  InBound(_4, T, 1)   ←  SMT: `(_5.term+1)*sizeof_T > len*sizeof_T` unsat by `_5.term < len`; `_5.term*sizeof_T < 0` unsat by `_5.term ≥ 0` (usize)    Proved
          check  Align(_4, T)        ←  align = 1 (concrete minimum for the unconstrained T)              Proved
...
_6 = get_unchecked_mut(move _7, move _8)                  [checkpoint (1)]
          ...
          VM  _6.term = _8.term + _7.term*sizeof_T, _6.prov = (data_alloc, _8.offset + _7.term*sizeof_T)

_5 = &mut (*_6)                                           [checkpoint (2)]
          check  Init(_6, T, 1)      ←  data_alloc.initialized = true — whole slice data is initialized at entry, so the element at offset _7.term*sizeof_T is covered    Proved
          check  NonNull(_6)         ←  flag: non_null (inherited from _8)                                Proved
          check  Allocated(_6, T, 1) ←  SMT: (_7.term+1)*sizeof_T ≤ len*sizeof_T                               Proved
          check  InBound(_6, T, 1)   ←  SMT: `(_7.term+1)*sizeof_T > len*sizeof_T` unsat by `_7.term < len`; `_7.term*sizeof_T < 0` unsat by `_7.term ≥ 0` (usize)    Proved
          check  Align(_6, T)        ←  align = 1 (concrete minimum for the unconstrained T)              Proved
