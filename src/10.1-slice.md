...
**Verification.** Entry facts: the precondition `InBound(index_access(self, index))`, plus the built-in `&[T]` slice invariant (`NonNull(ptr)`, `Align(ptr, T)`, `Init(ptr, T, len)`, …).

- Target (1): `SliceIndex::get_unchecked` requires `InBound(slice, self)` — exactly the entry precondition, discharged directly.
- Target (2): the `Ptr2Ref` checkpoint requires `Init(ptr, T, 1) && Align(ptr, T) && Alias(ptr)`. `Init` and `Align` are inherited from the source `&[T]` (whose invariant already carries `Init(ptr, T, len)` and `Align(ptr, T)`); the `Alias` hazard is auto-discharged because the source is shared — no mutable alias can coexist with a live shared reference (shared-from-shared is sound even for a non-`Copy` `T` such as `T: Drop`).
...
**Verification.** Entry facts: the precondition `InBound(index_access(self, index))`, plus the built-in `&mut [T]` slice invariant (including exclusive access).

- Target (1): discharged exactly as `get_unchecked` — the entry `InBound` matches `SliceIndex::get_unchecked_mut`'s `InBound(slice, self)`.
- Target (2): the `Ptr2Ref` checkpoint requires `Init(ptr, T, 1) && Align(ptr, T) && Alias(ptr)` — the same as `get_unchecked`, but the `Alias` hazard is now exclusive (no other alias at all). It is discharged by the exclusive `&mut [T]` receiver, whose exclusivity guarantees no other alias (shared or mutable) can coexist.
