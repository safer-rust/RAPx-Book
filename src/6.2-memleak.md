...
| File | Purpose |
|------|---------|
| `mod.rs` | Module entry, `rCanary` struct with config and `start()` entry point |
| `ranalyzer.rs` | Top-level analyzer: orchestrates heap ownership analysis and flow analysis |
| `ranalyzer/intra_visitor.rs` | Intra-procedural MIR visitor: constraint construction |
...
> Note: Analysis requires the `nightly` toolchain with `rustc-dev`, `rust-src`, and `llvm-tools-preview` components installed.
