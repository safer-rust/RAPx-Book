...
- **Deep matching support**: Now fully supported — nested enum destructuring (`Some(Ok(v))`), `switchInt` on dereferenced enum pointers (`*ptr`), and guard-clause comparison expansion (assert/if conditions propagate relational constraints to SMT via `BranchEq`).
