...
The MOP-based alias analysis (Chapter 5.2.2) is run first to compute alias summaries for all functions. SafeDrop shares the same `AliasGraph` infrastructure — the `safedrop` module reuses the alias analysis's `AliasGraph`, `Value`, and assignment tracking directly. The alias summaries identify which function parameters and return values may alias, enabling inter-procedural drop tracking without re-analyzing callees.
...
3. **Dangling Use Detection**: At each program point where a value is used (read, written, or passed to a function), the analysis checks whether any alias of that value has already been dropped along the current path. A warning is emitted if a dangling access is detected.
