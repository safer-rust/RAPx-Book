...
Below is a toy example demonstrating a Rust program with a use-after-free bug. The `Data` object is automatically dropped once the program exits the inner block scope of main. Accessing `ptr` after this point triggers a use-after-free error. You can find several such PoC examples within the folder of [`RAPx/tests`](https://github.com/safer-rust/RAPx/tree/main/tests).
...
The implementation can be found as a module named [SafeDrop](https://github.com/safer-rust/RAPx/blob/main/rapx/src/analysis/safedrop.rs).
