...
The MOP-based [alias analysis](./5.2-alias.md) is run first to compute alias summaries for all functions. `SafeDropGraph` wraps the alias analysis's `AliasGraph` and reuses its `Value`, `PtsGraph`, and `MopFnAliasPairs` types, along with the path-sensitive `alias_bb` and `alias_bbcall` traversal methods. The precomputed alias summaries identify which function parameters and return values may alias, enabling inter-procedural drop tracking without re-analyzing callees.

Key data structures from the alias analysis reused by SafeDrop:

- **`AliasGraph`**: Wraps `PathGraph` for CFG topology and path enumeration, plus alias-specific state (`values`, `pts_graph`, `move_sources`).
- **`Value`**: Represents a program place node (a local and its field projections via `father`/`fields`), indexed into the `PtsGraph` by `slot_idx`.
- **`PtsGraph`**: The unified points-to / value-flow graph. Drop metadata (`may_drop` / `need_drop`) and type classification (`slot_kind`) live on its slots; alias partitions are a union-find structure.
...
| `observer.rs` | Drop observer: records drop events during alias traversal |
| `drop.rs` | Drop point identification: determines where each value is dropped along a path |
| `checks.rs` | Bug pattern checks: use-after-free, double-free, and return-dangling detection |
