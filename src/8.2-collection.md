...
The `--prepare-targets` flag prints every **verification target** collected by the scanner — a function or method whose unsafe operations need to be checked — along with its resolved contracts. Each target's entry lists its struct invariants (if any), unsafe callees with their safety contracts, and the checkpoint paths through the control-flow graph. This is useful for checking whether a function is being picked up, or whether contracts are resolving as expected:
