...
This module presents an approach for verifying the soundness of unsafe code usage. 
...
RAPx supports three verification modes, selectable via the `--mode` flag:

#### 1. `scan` Mode (Default)
The `scan` mode is fully automatic. It scans the entire crate and identifies all functions that require verification — those containing internal `unsafe` code or whose associated struct defines type invariants. No manual annotation is needed. This mode is best suited for auditing unknown codebases.
...
#### 2. `targeted` Mode
The `targeted` mode only verifies functions explicitly annotated with `#[rapx::verify]`. This mode is designed for developers who want precise control over which functions to verify, reducing analysis time and focusing on specific safety-critical code paths.

```shell
cargo rapx verify --mode targeted
```

Functions must be annotated as follows:
```rust
#![feature(register_tool)]
#![register_tool(rapx)]

#[rapx::verify]
fn my_unsafe_function(ptr: *mut u32, len: usize) {
    unsafe {
        core::ptr::write(ptr.add(len - 1), 0);
    }
}
```

#### 3. `invless` Mode
The `invless` mode (invariant-less) verifies functions **without** relying on struct type invariants as pre/post-conditions. Instead, it derives safety requirements automatically from the safety flow graph. This mode is useful when struct invariants are not yet specified or when you want to discover implicit safety requirements.

```shell
cargo rapx verify --mode invless
```

> **Note:** You can also use `--prepare-targets` to list all `#[rapx::verify]` annotated functions and their safety contracts without running verification:
> ```shell
...
|---------|--------|------------|-----------|
| Annotation required | No | Yes (`#[rapx::verify]`) | No |
| Scope | All functions with unsafe code or struct invariants | Only annotated functions | All functions with unsafe code |
| Struct invariants | Used as pre/post-conditions | Used as pre/post-conditions | Ignored / auto-derived |
...
For the first two types of associated functions, if the verification target is a constructor, the verification obejctive is to ensure that:
$$SC_{\text{caller}} \cup \text{caller} \models SC_{\text{callee}} \cup SC_{\text{type}}$$

Otherwise, the verification obejctive is to ensure that:
$$SC_{\text{type}} \cup SC_{\text{caller}} \cup \text{caller} \models SC_{\text{callee}} \cup SC_{\text{type}}$$
...
The verification objectives discussed above is quite high-level. Further details can be found in [A Trace-based Approach for Code Safety Analysis](https://arxiv.org/pdf/2510.10410).

For [Challenge 17](https://model-checking.github.io/verify-rust-std/challenges/0017-slice.html), the verification targets are several unsafe methods.
...
```
cargo rapx verify -- --Zbuild-std --target x86_64-unknown-linux-gnu > /your/output/log/path/result.log 2>&1
```
