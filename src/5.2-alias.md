...
- **`alias_sets`**: Disjoint-set collections of value indices that are aliased. Each set is an equivalence class of aliased places.
...
1. **Construction**: `AliasGraph::new()` creates a `PathGraph` for the function and initializes `values` from MIR locals with drop and type information.
2. **Block Fact Extraction**: For each MIR basic block, `AliasGraph` extracts `AliasBlockFacts` — assignments (`_1 = copy _2`, `_1 = move _2`, `_1 = &_2`, `_1 = &raw _2`), aggregate initializations (tuples, structs decomposed field-by-field for field sensitivity), and constant values.
3. **SCC Decomposition**: `find_scc()` delegates to `PathGraph::find_scc()`, reusing the same SCC tree decomposition and top-down traversal strategy.
4. **Path-Sensitive Traversal**: `process_function_paths()` walks each enumerated path block-by-block. For each block, it calls:
...
5. **Result Merging**: After processing all paths, `merge_results()` collects alias pairs among function parameters and the return value, performs alias compression (field truncation and containment merging), and stores the result in `ret_alias`.
...
### 5.2.2.3 Feature: Path Sensitive with Light Reachability Constraints
...
[`PathGraph`](./5.1-path.md) provides `is_path_reachable()` and `check_segment_reachability_with()` to filter out such infeasible paths using discriminant constraint tracking. The alias analysis leverages this to avoid false aliases. For example:
...
- **With path filtering** ([Chapter 5.1](./5.1-path.md)): 2 reachable paths, result `{(0,1)}` — correctly excludes the impossible `y` alias.
...
1. **Lookup**: Checks `fn_map` for an existing callee summary. If found, uses it directly.
2. **Recursive Analysis**: If not found, recursively creates a new `AliasGraph` for the callee, runs `find_scc()` and `process_function_paths()`, caches the result, and uses it.
3. **Fixed-Point via Recursion Guard**: A `recursion_set` tracks functions currently being analyzed to prevent infinite recursion in the case of mutually recursive functions. If the callee is already in the `recursion_set`, it is conservatively skipped.
4. **Argument Mapping**: `handle_fn_alias()` maps callee-formal positions to caller-actual value indices, constructing field projections on-the-fly if the callee's alias references fields not yet materialized in the caller's value graph.
...
- **`sync_field_alias()`**: When two values are aliased, their corresponding fields are also aliased recursively (up to `MAX_FIELD_DEPTH = 10`).
...
1. **Graph construction**: Create `AliasGraph` wrapping a `PathGraph`, extract MIR locals into `values`, and extract per-block `AliasBlockFacts` from MIR statements and terminators.
2. **SCC decomposition**: Delegate to `PathGraph::find_scc()` for hierarchical SCC tree construction.
3. **Path-sensitive analysis**: `process_function_paths()` walks each path, calling `alias_bb()` and `alias_bbcall()` per block, building `alias_sets` along the way, then `merge_results()` extracts and compresses the final alias pairs.
...
1. **Kill Phase**: Remove all existing aliases involving `lv` and its field projections. Implemented via `remove_aliases_with_prefix()`.
2. **Gen Phase**: Establish new alias relationships based on the right-hand side.
...
1. **Collect Reachable Functions**: Starting from local crate functions, recursively traverse call sites to collect all reachable function `DefId`s.

2. **Initialize Summaries**: All reachable functions start with empty summaries (⊥ or `FnAliasPairs::new(arg_count)`).
