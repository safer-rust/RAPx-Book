...
The central data structure is `AliasGraph`, which wraps a `PathGraph` (Chapter 5.1) and layers alias-specific state on top:
...
The path sensitivity is achieved by processing each `PathGraph`-enumerated path independently: `process_function_paths()` maintains path-local `alias_sets` and `values` state, resetting between paths. The reachability filtering from `PathGraph` (Chapter 5.1) prunes infeasible paths early via `check_segment_reachability_with()`.
...
`PathGraph` (Chapter 5.1) provides `is_path_reachable()` and `check_segment_reachability_with()` to filter out such infeasible paths using discriminant constraint tracking. The alias analysis leverages this to avoid false aliases. For example:
