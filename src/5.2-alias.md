...
The MOP-based alias analysis achieves path sensitivity by enumerating distinct CFG paths and performing alias checks independently on each path, then merging results. The implementation lives at [`rapx/src/analysis/alias/default/`](https://github.com/safer-rust/RAPx/tree/main/rapx/src/analysis/alias/default/).

### 5.2.2.1 Architecture: AliasGraph and PtsGraph
...
- `values`: Per-local value nodes (locals and their field projections), kept for SafeDrop compatibility and indexed so each `Value` maps to a `PtsGraph` slot. Each `Value` records the originating MIR local, a `father` link and a `fields` map for field projections, and a `slot_idx` into the `PtsGraph`. Drop properties and type classification now live on the `PtsGraph` slots rather than on `Value`.
- `pts_graph`: The unified points-to / value-flow graph (see below), shared by the MoP alias analysis and SafeDrop.
- `move_sources`: Maps a Move-operand destination to its source value index, so SafeDrop can propagate drop information through move chains.
- `ret_alias`: The accumulated alias results for the function (MOP version with drop metadata).

The alias state lives in a **`PtsGraph`** (a unified points-to and value-flow graph), which replaced the earlier flat `alias_sets`/`block_facts` representation:

```rust
pub struct PtsGraph {
    points_to: Vec<FxHashSet<AbstractLoc>>, // slot holds a pointer into another slot
    value_flow: Vec<FxHashSet<usize>>,      // slot's value is a copy of another slot
    slots: Vec<Slot>,                       // registered slots (local + field path)
    may_drop: Vec<bool>,
    need_drop: Vec<bool>,
    slot_kind: Vec<ValueKind>,              // RawPtr, Ref, Adt, ...
    alias_parent: Vec<usize>,               // union-find alias partition
    may_hold: FxHashSet<(usize, usize)>,    // call return may hold any of several args
}
```

- `points_to`: directed edges recording that a slot holds a pointer/reference into another slot (created by `&_x`, `&raw _x`).
- `value_flow`: directed edges recording that a slot's value is a copy of another slot (created by `_a = _b`, casts).
- `alias_parent`: a union-find partition capturing value-equivalence through assignments. `may_alias` combines this partition with points-to intersection.
- `may_hold`: a call whose return value may be any one of several arguments keeps those arguments apart via `may_hold` edges instead of merging them into one partition.
...
1. `AliasGraph::new()` creates a `PathGraph` for the function and initializes `values` from MIR locals.
2. `init_pts_graph()` builds the `PtsGraph` from the body via `points_to::builder::from_body`, pre-registering every local and its type-determined field slots (up to depth limits) with `may_drop` / `need_drop` / `slot_kind`.
3. `PathGraph::find_scc()` computes the SCC tree decomposition, reusing the same top-down traversal strategy for loops.
4. `process_function_paths()` walks each enumerated path via `dfs_mop`. For each block it calls:
   - `alias_bb()`: Processes intra-block assignments (`process_assignment`), updating the `PtsGraph` — points-to edges (`&_x`), value-flow / alias-partition moves (`_a = _b`), and field-level propagation.
   - `alias_bbcall()`: Handles call terminators — resolves the callee's alias summary and applies it (`apply_callee_summary`), or falls back to a conservative merge for unknown calls.
5. At each path end, `merge_results_pts()` extracts field-sensitive alias pairs among parameters and the return value via `PtsGraph::fn_alias_pairs` (with field truncation/compression) and stores them in `ret_alias`.

The path sensitivity is achieved by processing each [`PathGraph`](./5.1-path.md)-enumerated path independently: `dfs_mop` saves and restores the `PtsGraph` between sibling paths, so every path is analyzed with a fresh alias state. The reachability filtering from `PathGraph` prunes infeasible paths early via `check_segment_reachability_with()`.
...
- `Value`/`Slot` projection: Each struct field access creates a field node. `Slot::project()` produces a field slot (`local.fields`), and `Value::projection()` recursively descends through `ProjectionElem::Field`, creating field nodes on demand (`father`/`fields` maps). `ProjectionElem::Deref` is skipped without materialising a new node, so multi-level pointer aliases are not tracked.
- Field-level propagation in `PtsGraph::assign_value`: when a slot's value is copied, the copy is propagated to the corresponding sub-field slots (every slot that extends the destination is linked to the matching slot that extends the source).
- Upward propagation in `PtsGraph::merge_equivalence`: when two slots are merged, `propagate_to_father` also unifies their parent slots one level up, so field-level aliases are visible through a base local.
- Slot registration bounds: `points_to::builder` pre-registers field slots up to `MAX_FIELD_DEPTH = 5` and `MAX_DEREF_DEPTH = 3`; the total number of slots per path is bounded by `MAX_VALUES_PER_PATH = 1000`.
- Alias compression: after path traversal, `fn_alias_pairs` performs field truncation (`compress_fields`, collapsing multi-level field projections to the first level) and containment merging (removing redundant sub-field aliases when a coarser alias already captures the relationship).
...
1. Create `AliasGraph` wrapping a `PathGraph`, extract MIR locals into `values`, and build the `PtsGraph` from the body (`init_pts_graph`).
2. Delegate to `PathGraph::find_scc()` for hierarchical SCC tree construction.
3. `process_function_paths()` walks each path via `dfs_mop`, calling `alias_bb()` and `alias_bbcall()` per block to update the `PtsGraph`, then `merge_results_pts()` extracts and compresses the final alias pairs.
...
In addition to the MOP approach, RAPx provides an alternative alias analysis implementation based on the **MFP (Maximum Fixed Point)** framework. The MFP analysis is built on Rust compiler's `rustc_mir_dataflow` infrastructure, which provides a general-purpose monotone dataflow analysis framework. The implementation lives at [`rapx/src/analysis/alias/mfp/`](https://github.com/safer-rust/RAPx/tree/main/rapx/src/analysis/alias/mfp/).
