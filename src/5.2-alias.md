...
## 5.2.1 Alias Analysis Trait
RAPx provides the [`AliasAnalysis`](https://docs.rs/rapx/latest/rapx/analysis/alias_analysis/trait.AliasAnalysis.html) trait for alias analysis. The trait has several methods, which enables users to query the aliases among the arguments and return value of a function based on the function `DefId`, or the aliases of all functions as a `FxHashMap`. Developers can implement the trait based on their needs.
```rust
...
## 5.2.2 Default Alias Analysis
The MOP-based alias approach is achieved via a struct `AliasAnalyzer`, which implements the `AliasAnalysis` trait. The detailed implementation can be found in [mop.rs](https://github.com/safer-rust/RAPx/blob/main/rapx/src/analysis/alias_analysis/default/mop.rs). 
```rust
...
### 5.2.2.1 Feature: Path Sensitive with Light Reachability Constraints

The MOP-based alias analysis achieves path sensitivity by enumerating distinct CFG paths and performing alias checks independently on each path, then merging results. However, not all structurally possible paths are actually executable — correlated branch conditions on the same discriminant can make certain path combinations unreachable.

PathGraph (Chapter 5.1) provides `is_path_reachable()` to filter out such infeasible paths using discriminant constraint tracking. The alias analysis leverages this to avoid false aliases. For example:
...
- **With path filtering** (Chapter 5.1): 2 reachable paths, result `{(0,1)}` — correctly excludes the impossible `y` alias.

The alias analysis uses `PathGraph::check_segment_reachability_with()` to propagate discriminant constraints across SCC boundaries during top-down traversal, ensuring that inner SCC paths are only explored when consistent with outer context constraints.

### 5.2.2.2 SCC Handling (default)
A major challenge for path-sensitive analysis lies in the presence of strongly connected components (SCCs), since loops may induce an unbounded number of execution paths. The alias analysis addresses this by leveraging the SCC tree decomposition and path enumeration infrastructure provided by the Path Analysis module (see Chapter 5.1). Key observations specific to alias analysis:
...
The annotated MIR for this function contains approximately 27 basic blocks with the following structure:

- **bb0**: Entry block — initializes `r = x`, `q = x`, branches to bb1.
- **bb1** (outer loop header / outermost SCC dominator): Checks `*r > 0`; if false, exits to return block.
- **bb22 → bb2 → bb4**: Sets `p = match choice { First => bb5(y), Second => bb6(x) }`.
- **bb5 / bb6**: Assign `p = y` or `p = x` respectively, flow to bb7.
- **bb7** (inner SCC dominator): Entry of the nested loop `loop { ... }`.
- **bb8 / bb9**: `r = match choice { First => bb8(x), Second => bb9(p) }`.
- **bb10**: `q = y`, flow to bb13.
- **bb11 / bb12**: `p = q`, flow to bb13.
- **bb13**: `*q -= 1`, then check `*r <= 1` → break to bb15 or continue back to bb7.
- **bb15**: Check `*r == 0` → break out of outer loop or jump back to bb1 (outer SCC re-entry).

The SCC tree has two levels: the outer SCC is dominated by bb1 with sub-SCC dominated by bb7.

The CFG contains a large SCC whose dominator is block `1`, and a nested sub-SCC with dominator block `7`, forming a two-level SCC tree (see Chapter 5.1 for the underlying decomposition algorithm).

We enumerate paths through the SCC tree in a recursive, top-down manner. A path segment between two occurrences of the same dominator is worth exploring only if it introduces at least one previously unvisited node. For instance, the segment `7 → 8 → 10 → 21 → 24 → 11 → 20 → 25 → 19 → 13 → 7` followed by `7 → 9 → 10 → 21 → 24 → 11 → 20 → 25 → 19 → 13 → 7` is valuable because the second repetition introduces block `9`, which may lead to new alias relationships. The resulting extended path is `7-8-10-21-24-11-20-25-19-13-7-9-10-21-24-11-20-25-19-13-7-8-10-21-24-11-20-25-19-26`.

Using this strategy adjusted for alias analysis (postfix repeat = 0, each distinct segment appears at most once), the inner SCC produces the following 10 paths:
...
The alias analysis adopts a recursive, top-down strategy (see Chapter 5.1) that propagates accumulated path constraints from outer SCCs inward. When reaching an inner SCC, constraints from the outer context determine which of the enumerated paths are reachable. For each reachable path, the inner SCC subsequence is concatenated with the outer SCC path, and alias analysis continues along the resulting trace.


### 5.2.2.3 SCC Handling with Fix-Point (Experimental)

The standard SCC handling (Section 5.2.2.2, backed by the PathGraph infrastructure in Chapter 5.1) works well when loop iterations do not change alias relationships. However, some programs exhibit **loop-carried state propagation** where each iteration modifies pointer assignments in ways that affect subsequent iterations. This section presents an experimental extension that addresses this case.
...
#### Case Study
The inner SCC of the motivating example has the following CFG structure:

- **bb6** (inner SCC dominator): Entry point, branches based on `random()` — either exits to bb8 or continues to `match choice`.
- **bb7**: `r = match choice { First => bb8(x), Second => bb10(p) }`.
- **bb9**: `p = q`, flow to bb11.
- **bb10**: Flow to bb11.
- **bb11**: Call `random()` → exit to bb3 or continue to bb12.
- **bb12**: `p = q`, flow back to bb6 (SCC re-entry).

The two `choice`-dependent branches (bb8 for `First`, bb10 for `Second`) are never reachable together in a single execution trace — the immutable `choice` value is inherited as a constraint from the outer SCC context.
...
### 5.2.2.4 Feature: Field Sensitive 
...
There are three key steps ([source code](https://github.com/safer-rust/RAPx/blob/main/rapx/src/analysis/alias_analysis/default/mop.rs)):
```rust
...
* **Graph preparation**: Construct the control-flow graph for the target function. See the [source code](https://github.com/safer-rust/RAPx/blob/main/rapx/src/analysis/alias_analysis/default/graph.rs).
* **SCC shrinkage**: Extract the strongly connected components (SCCs) and shrink SCCs of the control-flow graph. See the [source code](https://github.com/safer-rust/RAPx/blob/main/rapx/src/analysis/alias_analysis/default/graph.rs).
* **Alias Check**: Traversal the control-flow graph and perform alias analysis. See the [source code](https://github.com/safer-rust/RAPx/blob/main/rapx/src/analysis/alias_analysis/default/mop.rs)
...
The MFP alias analyzer is implemented in the `MfpAliasAnalyzer` struct ([source code](https://github.com/safer-rust/RAPx/blob/main/rapx/src/analysis/alias_analysis/mfp/mod.rs)), which also implements the `AliasAnalysis` trait. Like the MOP approach, MFP analysis is field-sensitive and context-insensitive, supporting interprocedural analysis through function summaries.
