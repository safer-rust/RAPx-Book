...
`Init` means `_17` points to initialized（non-`MaybeUninit`）memory that fits within its allocation.

- **Initialized?** Yes: `KnownInit` from `Box::new` marks `_17`'s origin as initialized.
- **Fits within allocation?** `Box::new` allocates one `Node`, so the allocation starts at `_17` with space for `1` element. The access is `1` element starting at offset `0`:

```
offset + access = 0 + 1 ≤ 1
```

The goal `0 + 1 ≤ 1` holds trivially. The solver asserts `¬(0 + 1 ≤ 1)` alongside the allocation bounds — the contradiction is immediate → `Unsat` → `Init | Proved`.
