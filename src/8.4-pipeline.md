...
`pointer_bounds_for_place` traces through `PointsTo` to find the allocation bounds for `_17`. Since `Box::new` returns a pointer to the start of a fresh allocation, `offset(_17) = 0`; the allocation holds `1` element of `size_of::<Node>()`, so `len ≥ 1`. `origin_is_initialized_for_ty` confirms the allocation is not `MaybeUninit`.

The goal is `0 ≤ offset ∧ 0 ≤ access ∧ offset + access ≤ len` — the access（`access = 1`）must fit within the allocation:

```
0 ≤ 0  ∧  0 ≤ 1  ∧  0 + 1 ≤ len
```

Since `len ≥ 1`, all three hold. The solver checks `constraints ∧ ¬goal` → `Unsat` → `Init | Proved`.
