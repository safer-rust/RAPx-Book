...
For the checkpoint `bb10 -> as_mut`, Ptr2Ref requires two preconditions: `Init` and `Align`. At repeat 0, the path extractor produces paths reaching bb10 on different loop iterations. Both preconditions are proved on every path:
...
But `as_mut` is called inside a loop — could deeper unrolling expose a violation or reveal missing facts? The auto-detector tries repeat 1, adding one extra SCC repetition:
...
The `Proved` set（`Init` and `Align` on every path）is identical between repeat 0 and repeat 1. The detector sees convergence and stops — no hidden bugs, no missing facts, and deeper unrolling adds no new information.
