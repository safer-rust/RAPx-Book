...
`p1 == p2` — the two places refer to the same memory. A hazard by default: unproved `Alias` is reported as a `[hazard]` entry and contributes to `UNSOUND`. It is also the mechanism for a no-aliasing *precondition* — `Ptr2Ref(p, T)` (and `Drop(p, T)`) include `Alias(p)`, and a caller declares `#[rapx::requires(Ptr2Ref(p, T))]` to discharge the callee's hazard. Alias itself is not a safety violation, provided it does not also violate `Owning` or `Alive`. [psp IV.2](https://github.com/safer-rust/safety-tags/blob/main/primitive-sp.md#342-alias).
...
A property may carry `kind = "precond" | "hazard" | "option"` metadata (e.g. `#[rapx::requires(..., kind = "hazard")]`); `Alias` is always classified `hazard`, and its *precondition* role is expressed through `Ptr2Ref`/`Drop`, which include `Alias(p)`.
